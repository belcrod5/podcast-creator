...
`Contents/MacOS/Podcast Creator` の直接実行は、Aqua セッション外の CLI 実行環境では macOS/AppKit 初期化で即終了することがあります。  
配布物を CLI から安定して呼ぶ場合は、パッケージ後に同じフォルダへ生成される `podcast-creator-cli` を使ってください。

```
./release/Podcast\ Creator-darwin-arm64/podcast-creator-cli \
  --podcast /path/to/podcast.json \
  --workdir /path/to/workdir
```
