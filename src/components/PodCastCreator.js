...
const normalizeScriptItems = (items = []) =>
    items.map((original = {}) => {
        ...
        const fallbackPath = typeof item.path === 'string' && item.path ? item.path : (typeof item.videoPath === 'string' ? item.videoPath : '');
...
const PodCastCreator = () => {
    ...
    }, [pendingPreset, bgmList, selectedYoutubeToken, setYoutubeTokenFile, setSelectedBgm, setBgmVolume, setSelectedIntroBgVideo, setFixedYoutubeDescription, checkYoutubeAuthStatus, youtubeTokenFiles, isYoutubeCredentialsAvailable, autoUploadToYoutube]);
    ...
    const handleJsonPaste = async () => {
        try {
            ...
            if (data.script && data.script.length > 0) {
                // スクリプトアイテムを設定
                const scriptData = normalizeScriptItems([...data.script]);
    ...
    useEffect(() => {
        ...
        try {
            if (formData.script.text) {
                const parsedItems = normalizeScriptItems(JSON.parse(formData.script.text));
    ...
    const updateScriptItems = (items) => {
        const normalized = normalizeScriptItems(items);
    ...
    const adjustInsertVideoTimings = useCallback(async (options = {}) => {
        ...
        try {
            ...
            if (!parsed || !Array.isArray(parsed.script)) {
                alert('マッピングデータにscript配列が存在しません');
                return;
            }

            const mappingEntries = parsed.script;
            if (!mappingEntries.length) {
                alert('マッピングデータのscript配列が空です');
                return;
            }
            ...
            const findClosestMatch = (isStart, targetSeconds) => {
                ...
                if (bestCandidate && bestDiff > INSERT_VIDEO_MATCH_TOLERANCE_SECONDS) {
